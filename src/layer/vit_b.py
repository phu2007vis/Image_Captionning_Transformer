...
class ImageEncoderViT(nn.Module):
	...
	def custom_load_state(self, state, prefix='model.vision_tower_high.', verbose=False):
		"""
		Custom method to load specific keys from a state dictionary into the current model's state dict.

		Args:
			state (dict): The state dictionary to load from.
			prefix (str): The prefix to be replaced in keys from the incoming state.
			verbose (bool): If True, prints detailed loading logs.
		...
		for key, value in state.items():
			if key.startswith(prefix):
				# Transform the key to match the current model's keys
